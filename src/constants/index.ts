export const THEME_STORAGE_KEY = "dex-theme";

export const UNKNOWN_SPRITE_URL =
  "data:image/png;base64,";

export const STAT_TYPES = [
  "HP",
  "Atk",
  "Def",
  "SpA",
  "SpD",
  "Spe",
  "BST",
] as const;

export const FILTER_TYPES = [
  "Name",
  "Type",
  "Ability",
  "Level Up Moves",
  "TMS",
  "Egg Moves",
  "Tutor Moves",
] as const;

export const POKEMON_TYPES = [
  { name: "Normal", color: "#A8A77A" },
  { name: "Fighting", color: "#C22E28" },
  { name: "Flying", color: "#8571BE" },
  { name: "Poison", color: "#A33EA1" },
  { name: "Ground", color: "#B1954F" },
  { name: "Rock", color: "#B6A136" },
  { name: "Bug", color: "#808F13" },
  { name: "Ghost", color: "#735797" },
  { name: "Steel", color: "#8C8C9E" },
  { name: "Fire", color: "#EE8130" },
  { name: "Water", color: "#6390F0" },
  { name: "Grass", color: "#62A13D" },
  { name: "Electric", color: "#CF9A09" },
  { name: "Psychic", color: "#F95587" },
  { name: "Ice", color: "#65A19E" },
  { name: "Dragon", color: "#6F35FC" },
  { name: "Dark", color: "#705746" },
  { name: "Fairy", color: "#D685AD" },
];
